...
- **[#28305](https://github.com/ggml-org/llama.cpp/pull/28305)** (open) - **in the build, unmeasured on its own terms.** Keeps the backend sampling subgraph a fixed shape across ubatches so the spec-dec verify step (variable accepted-window row count) stops triggering ggml-alloc reallocation. We run `spec-draft-backend-sampling = 1`. Prefill-null as expected; the effect, if any, is on decode and has not been measured.
- **[#28439](https://github.com/ggml-org/llama.cpp/pull/28439)** (open, draft) - **in the build, null here.** Metal FA query tile 8 -> 16 rows when `ne01 >= 64` and the head size pads to a multiple of 128; our DK=DV=256 qualifies. The author's M5 hs-256 wins (-34% at 16K, -47% at 64K KV) do not show at 33K cold prefill because the vec and sparse paths are untouched and `patches/0002` already routes the QSA layers through the sparse kernel. Kept because it is free and may matter at 64K+; check with `llama-bench -d 65536,131072` before relying on it.
