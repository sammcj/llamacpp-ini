...
Needs `llama-server` and `llama-gguf` on PATH, Python 3, and `llama-diffusion-cli` for diffusion models.
...
Diffusion models (dream, llada, diffusion-gemma) can't load in llama-server, so they run separately:

```sh
./run-diffusion.sh --list
./run-diffusion.sh <model> -p "your prompt"
```
...
`LLAMA_SERVER_BIN`, `LLAMA_DIFFUSION_BIN`, `LLAMA_GGUF`, `LLAMA_BASE_INI`, `MODELS_SRC` (default `~/.lmstudio/models`), `MODELS_DIR` (default `./models`), `SKIP_SYNC`.
