...
- After pulling llama.cpp master, run `./update-mtp-build.sh` to bring the worktree along: it fetches the latest PR head, merges origin/master and each PR in `EXTRA_PRS` (falls back gracefully when any of them stops merging cleanly), rebuilds, and exits early when nothing changed. It also detects the PR merging upstream and prints the retirement steps. A `gh`-backed state check warns when a listed PR was closed without merging, or was squash-merged upstream so the ancestry test cannot see it; both cases would otherwise be re-merged silently forever. It builds with the main repo's `local` cmake preset (LTO, native; symlinked into the worktree since `CMakeUserPresets.json` is untracked) and never runs `cmake --install`, so the PR binaries stay out of `~/.local`.
...
- **#27941 follow-up fixes: merged upstream 2026-09-01, now arrives via master.** Squash-merged as `36b101543`, so the script's ancestry test never saw it and it was being re-merged on every run until the state check caught it; removed from `EXTRA_PRS`. A/B when it was added measured 56.68 vs 56.70 t/s and identical acceptance - no gain, no regression, single stream. It was kept for two reasons that do not show up in a one-request benchmark: blocks are keyed per sequence rather than by position alone (wrong under `kv-unified`, which is what the router's 4 auto slots run), and eight `GGML_ASSERT` sites reachable from a hand-edited GGUF become throws - this model *is* hand-edited.
...
- **#28121 noscan ssm_a flags: merged upstream 2026-09-01, now arrives via master.** Squash-merged as `f28493c78`; same story as #27941, removed from `EXTRA_PRS`. 4 lines, never measured separately.
